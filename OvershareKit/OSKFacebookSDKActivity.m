...
static NSString * OSKFacebookSDKActivity_UserInfoKey = @"OSKFacebookSDKActivity_UserInfoKey";

@implementation OSKFacebookSDKActivity
...
- (BOOL)isAuthenticated {
    return FBSession.activeSession.isOpen && self.userInfo;
}
...
{
    // lazily load user info from user defaults
    if (!_userInfo && FBSession.activeSession.isOpen)
    {
        _userInfo = [[NSUserDefaults standardUserDefaults] objectForKey:OSKFacebookSDKActivity_UserInfoKey];
    }
    return _userInfo;
}

- (void)getUserDetailsFromCache:(BOOL)fromCache completion:(OSKGenericAuthenticationCompletionHandler)completion
{
    [[FBRequest requestForMe] startWithCompletionHandler:
     ...
     {
         self.userInfo = user;
         [[NSUserDefaults standardUserDefaults] setObject:self.userInfo forKey:OSKFacebookSDKActivity_UserInfoKey];
         
         if (!error && user)
         {
             if ([[OSKActivitiesManager sharedInstance].customizationsDelegate respondsToSelector:@selector(activityDidAuthorize:)])
             {
                 [[OSKActivitiesManager sharedInstance].customizationsDelegate activityDidAuthorize:self];
             }
         }
         
         if (completion)
         {
             completion(!error, fromCache, error);
         }
     }];
}
...
- (void)requestPublishPermissionsWithCompletion:(OSKGenericAuthenticationCompletionHandler)completionBlock
{
    [FBSession.activeSession requestNewPublishPermissions:[NSArray arrayWithObject:@"publish_actions"]
                                        ...
                                        completionHandler:^(FBSession *session, NSError *error)
     {
         if (completionBlock)
         {
             completionBlock((session.isOpen && [session.permissions indexOfObject:@"publish_actions"] != NSNotFound), NO, error);
         }
     }];
}
...
+ (NSString *)activityType {
    return OSKActivityType_iOS_FacebookSDK;
}
...
- (void)performActivity:(OSKActivityCompletionHandler)completion {
    __weak OSKFacebookSDKActivity *weakSelf = self;
    ...
    {
        [self requestPublishPermissionsWithCompletion:^(BOOL successful, BOOL fromCache, NSError *error)
         {
             if (successful)
             {
                 executePost();
